...
public class NWS : MonoBehaviour
{
    [Header("Crosshair")]
    ...
    public ShotType weaponType = ShotType.Manual;
    ...
    private void Update()
    {
        ...
        if (Input.GetKey(KeyCode.Mouse0) && weaponType == ShotType.Automatic)
        {
            TryShoot();
        }

        if (Input.GetKeyUp(KeyCode.Mouse0) && weaponType == ShotType.Automatic)
        {
            shotCounter = 0;
        }
    ...
    private bool TryShoot()
    {
        ...
        if (weaponType == ShotType.Automatic)
        {
            if (shotCounter > 3)
                RecoilMath();
        }
